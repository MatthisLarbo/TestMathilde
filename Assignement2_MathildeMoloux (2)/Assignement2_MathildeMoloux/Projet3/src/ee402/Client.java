...
public class Client {
    ...
    public Client(String serverIP, String clientID) {
    ...
    private void getDate() {
    	///sending object of type TransferData 40 times to the server
    	for(int i=0; i<40; i++)
    	{
	    	TransferData data = new TransferData(this.clientID);
	    	int reception = 1000;
	
	    	System.out.println("01. -> Sending Command (" + data.getTemp() + ") to the server...");
	
	    	this.send(data);
	    	try{
	///receive the frequency
	    		reception = (int) receive();
